...
\begin{figure}[!t]
	\centering\includegraphics[width=1\textwidth]{images/pipeline}
...
\textbf{{Input: Stochastic Workflow Nets}.} Stochastic Workflow Nets represent the first possible input that can be fed to the pipeline. These are customary formal basis \cite{DBLP:conf/bpm/LeemansSA19,DBLP:conf/icpm/PolyvyanyyK19,DBLP:journals/tosem/PolyvyanyySWCM20} to represent processes \cite{MarsanCB84,RoggeSoltiAW13}. We consider an interesting class of stochastic Petri nets with no times transitions, namely \textsc{untimed Stochastic Workflow Nets}.
...
The notions of transition enablement and firing are also the standard ones, since they do not depend on the weight function. %, which provides the basis for capturing the stochastic behavior of the net. 
...
produce the same consequent state:%. In this case, 
