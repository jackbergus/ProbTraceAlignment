...
As sketched in \figurename~\ref{fig:pipe}, our approach takes as input
...
\textbf{{Input: Stochastic Workflow Nets}.} Petri Nets and Generalized Stochastic Petri Nets are well-established formalisms for modelling processes;  they are customary formal basis \cite{DBLP:conf/bpm/LeemansSA19,DBLP:conf/icpm/PolyvyanyyK19,DBLP:journals/tosem/PolyvyanyySWCM20} to represent processes \cite{MarsanCB84,RoggeSoltiAW13}. Due to the lack of space, we refer to \cite{spdwe} for the usual notation over Petri Nets. We restrict Generalized Stochastic Petri Nets to an interesting class of stochastic Petri nets with no times transitions, namely \textsc{untimed Stochastic Workflow Nets}. We assume to have a set $\alphabet = \tasks \cup \set{\tau}$ of labels, where labels in $\tasks$ indicate process tasks, whereas $\tau$ indicates an invisible execution step ($\tau$-transition). A \emph{trace} is a finite sequence of labels from $\tasks$.
...
The current state of execution is captured using a marking, i.e., a multiset of places $P$ indicating how many tokens populate each place.
...
The notions of transition enablement and firing are also the standard ones, since they do not depend on the weight function \cite{MarsanCB84}: %, which provides the basis for capturing the stochastic behavior of the net. 
...
\textbf{Input: Transition Graphs.} %\label{subsec:ppn}
...
rely on probabilistic transition graphs \cite{GartnerFW03}. Such graphs have transition probabilities associated to the edges, while nodes have labels in $\Sigma$, 
...
\textbf{Transforming SWN$\to$TG.} If input is provided as a SWN,  we represent all transition firings of an \uswn, together with their probabilities, in a reachability graph by interpreting concurrency by interleaving.
\begin{definition}
...
\end{compactitem}
\end{definition}
We now need to consider that, in a given state, distinct net transitions with the same label produce the same consequent state, thus becoming indistinguishable from the trace perspective as they collaps into a single edge of the reachability graph. So, we accumulate all their firing probabilities into a single edge value. 
