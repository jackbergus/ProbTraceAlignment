...
We now describe the proposed technique for computing probabilistic trace alignments. As sketched in \figurename~\ref{fig:pipe}, our approach takes as input
...
\textbf{{Input: Stochastic Workflow Nets}.} Petri Nets and Generalized Stochastic Petri Nets are well-established formalisms for modelling processes; in fact, they are customary formal basis \cite{DBLP:conf/bpm/LeemansSA19,DBLP:conf/icpm/PolyvyanyyK19,DBLP:journals/tosem/PolyvyanyySWCM20} to represent processes \cite{MarsanCB84,RoggeSoltiAW13}. Due to the lack of space, we refer to \cite{spdwe} for the usual notation over Petri Nets. We restrict Generalized Stochastic Petri Nets to an interesting class of stochastic Petri nets with no times transitions, namely \textsc{untimed Stochastic Workflow Nets}. We assume to have a set $\alphabet = \tasks \cup \set{\tau}$ of labels, where labels in $\tasks$ indicate process tasks, whereas $\tau$ indicates an invisible execution step ($\tau$-transition). A \emph{trace} is a finite sequence of labels from $\tasks$.
...
Last, the resulting $\rg{\net}$ (\figurename~\ref{fig:rg}) is converted into a corresponding transition graph $\tg_{\rg{\net}}$ (\figurename~\ref{fig:orig}) preserving model traces as well as their probabilities. We omit the details of this conversion, due to lack of space and the fact that it employs well-known techniques used to \emph{shift labels} from transitions to states while preserving the behavior encoded by a transition system.
\medskip
